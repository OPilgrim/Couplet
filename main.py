...
def get_args():
    ...
    parser.add_argument("-m", "--model", default='transformer', type=str)   # 可选 BiLSTM, Transformer, CNN, BiLSTMAttn, BiLSTMCNN, BiLSTMConvAttRes
    parser.add_argument("--fp16", action='store_true')    # fp16半精度浮点数
    parser.add_argument("--fp16_opt_level", default='O1', type=str)   # opt_level,00纯fp32，可作为精度的baseline；01混合精度训练；02几乎fp16的混合精度训练；03纯fp16，不稳定，可作为speed的baseline
...
def auto_evaluate(model, testloader, tokenizer):
    ...
    for step, batch in enumerate(testloader):
        ...
        target_ids = batch[-1]
        with torch.no_grad():   # 不要进行梯度训练我记得
        ...
        for seq, tag in zip(preds.tolist(), target_ids.tolist()):
            ...
            bleu = calc_bleu(seq, tag)   # 计算bleu分数
            rl = calc_rouge_l(seq, tag)  # 计算 不知道是什么的分数？？？？
            bleus.append(bleu)
            rls.append(rl)
    return sum(bleus) / len(bleus), sum(rls) / len(rls)    # 平均的bleu值和rls什么鬼？
...
def run():
    ...
    if args.fp16:
        try:
            ...
            amp.register_half_function(torch, 'einsum')  # 某些不常用的函数，在使用前需要注册；某些函数（如einsum）暂不支持FP16加速，建议不要用的太heavy
            model, optimizer = amp.initialize(model, optimizer, opt_level=args.fp16_opt_level)
        except ImportError:
            raise ImportError("Please install apex from https://www.github.com/nvidia/apex to use fp16 training.")
    if torch.cuda.device_count() > 1:
        model = torch.nn.DataParallel(model)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min')   # 当网络的评价指标不在提升的时候，可以通过降低网络的学习率来提高网络性能。
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            optimizer.step()
            if step % 100 == 0:
                tb.add_scalar('loss', loss.item(), global_step)      # tensorboard画图用的
                logger.info(
