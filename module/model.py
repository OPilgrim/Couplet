...
class Transformer(nn.Module):
    def __init__(self, vocab_size: int, max_seq_len: int, embed_dim: int, hidden_dim: int, n_layer: int, n_head: int, ff_dim: int, embed_drop: float, hidden_drop: float):
        ...
        layer = nn.TransformerEncoderLayer(
            d_model=hidden_dim, nhead=n_head, dim_feedforward=ff_dim, dropout=hidden_drop)   # 由self-attn和feedforward组成，此标准编码器层基于“Attention Is All You Need”一文
        self.encoder = nn.TransformerEncoder(layer, num_layers=n_layer)    # N个编码器层的堆叠
